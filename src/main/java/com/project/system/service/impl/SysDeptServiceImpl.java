...
public class SysDeptServiceImpl extends BaseServiceImpl<SysDeptServiceImpl> implements ISysDeptService {
    ...
    public List<Map<String,Object>> childrensMap(long deptId, Integer type) {
        List<SysDept> list=childrens(deptId,type);
        List<Map<String, Object>> mapList=new ArrayList<>();
        list.forEach(sysDept -> {
            String json= JSONObject.toJSONString(sysDept);
            //删除为空的子级列表
    ...
    public List<SysDept> childrens(long detpId, Integer type) {

        List<SysDept> list=deptMapper.selectDeptList(new SysDept());
        list=childrensTree(list,detpId,type,new ArrayList<>());
        return list;
    }
     ...
     * @return
     */
    public  List<SysDept> childrensTree(List<SysDept> list,long deptId,Integer type,List<SysDept> newList){
        list.forEach(sysDept -> {
            if(type==1){
                if(sysDept.getDeptId()==deptId){
                    System.err.println(sysDept.getDeptName());
                    sysDept.setChildrens(childrensTree(list,sysDept.getParentId(),type,new ArrayList<>()));
                    newList.add(sysDept);
                }
            }else if(type==2){
                if(sysDept.getParentId()==deptId){
                    sysDept.setChildrens(childrensTree(list,sysDept.getDeptId(),type,new ArrayList<>()));
                    newList.add(sysDept);
                }
            }else if(type==3){
                if(sysDept.getParentId()==deptId){
