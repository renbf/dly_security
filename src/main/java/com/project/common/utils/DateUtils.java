...
public class DateUtils extends org.apache.commons.lang3.time.DateUtils {
    ...
    /**
     *  忽略时分秒毫秒 后 计算两个时间的天数间隔, date - date1
     *  相当于mysql中的函数 DATEDIFF(date,date1)
     * @param date
     * @param date1
     * @return
     */
    public static int getIntervalDays(Date date, Date date1) {
        DateFormat dateFormat = new SimpleDateFormat("yyyyMMdd");
        Date leftDate = null;
        Date rightDate = null;
        try {
            leftDate = dateFormat.parse(dateFormat.format(date));
            rightDate = dateFormat.parse(dateFormat.format(date1));
        } catch (ParseException e) {
            e.printStackTrace();
        }
        return (int) ((leftDate.getTime() - rightDate.getTime()) / (1000 * 60 * 60 * 24));
    }
